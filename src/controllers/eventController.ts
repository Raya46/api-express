...
async function getCalendarEventsInternal(req: Request, res: Response, calendarId: string) {
  try {
    ...
    const {
      ...
      orderBy = "startTime",
      q, // search query
      updatedMin, // only get events updated after this time
    } = req.query;
    ...
    if (updatedMin) queryParams.updatedMin = updatedMin as string;
...
// FIXED: New function to get events from specific calendar
